...
async function notify_admin_display_status_changed(app, ws, ws_online) {
	app.db.tournaments.findOne({ key: default_tournament_key }, async (err, tournament) => {
		if (err || !tournament) {
			err = err || { message: 'No tournament ' + default_tournament_key };
			return;
		}
...
async function handle_score_update(app, ws, msg) {
	...
	async.waterfall([
		...
		(match, court, cb) => {
			if (!match || !court) {
				return cb(null, match, court, false);
			}
...
async function update_device_info(app, tournament_key, device_info) {
	app.db.tournaments.findOne({ key: tournament_key }, async (err, tournament) => {
		if (err || !tournament) {
			err = err || { message: 'No tournament ' + tournament_key };
			return;
		}
...
function handle_score_change_all_courts(app, tournament_key) {
	const court_ids = new Set();
	for (const panel_ws of all_panels) {
		if (panel_ws.court_id) court_ids.add(panel_ws.court_id);
	}
	for (const cid of court_ids) {
		matches_handler(app, null, tournament_key, cid);
	}
	if (all_matches_delivery()) {
		matches_handler(app, null, tournament_key, undefined);
	}
...
function matches_handler(app, ws, tournament_key, court_id) {
	...
	app.db.fetch_all([{
		...
		if(db_matches){
			...
			matches = matches.filter(m => m.setup.state == 'oncourt' || m.setup.state == 'finished' || m.setup.state == 'blocked' || m.setup.called_to_court);
			if (tournament.courts_to_call_enabled) {
				matches = matches.filter(m => m.setup.called_to_court || m.setup.state == 'finished');
			}
			...
			const battery_by_court = {};
			for (const panel_ws of all_panels) {
				if (panel_ws.court_id && panel_ws.battery) {
					battery_by_court[panel_ws.court_id] = panel_ws.battery;
				}
			}
			event.battery_by_court = battery_by_court;

			const ctc = !!tournament.courts_to_call_enabled;
			event.call_settings = {
				courts_to_call_enabled: ctc,
				second_call_enabled: ctc && (tournament.second_call_enabled !== false),
				second_call_s: tournament.second_call_s || 420,
				final_call_enabled: ctc && (tournament.final_call_enabled !== false),
...
function create_event_representation(tournament) {
	...
	if (tournament.tshirt_enabled) {
		res.tshirt_enabled = true;
		res.tshirt_sizes = tournament.tshirt_sizes || 'XS,S,M,L,XL';
	}
	return res;
}
...
async function change_default_display_mode(app, tournament, old_displaysettings_id, new_displaysettings_id) {
	if (new_displaysettings_id) {
		app.db.display_court_displaysettings.find({ displaysetting_id: old_displaysettings_id }).exec( async (err, display_court_displaysettings) => {
			if (err) {
				console.error('Error in change_default_display_mode:', err);
				return;
			}
			const updatevalues = {
				displaysetting_id: new_displaysettings_id
...
module.exports = {
	...
	handle_command_done,
	handle_score_change,
	handle_score_change_all_courts,
