...
async function integrate_matches(app, tkey, btp_state, location_map, court_map, callback) {
	...
	const t_settings = await new Promise((res, rej) => {
		app.db.tournaments.findOne({key: tkey}, (err, t) => err ? rej(err) : res(t));
	});
	const auto_call = t_settings && !t_settings.courts_to_call_enabled;
	const second_call_ms = ((t_settings && t_settings.second_call_s) || 420) * 1000;
	const final_call_ms = ((t_settings && t_settings.final_call_s) || 300) * 1000;
	...
	async.each(btp_state.matches, function (bm, cb) {		
		...
		app.db.matches.findOne(query, (err, cur_match) => {
			...
			craft_match(app, tkey, btp_id, location_map, court_map, event, draw, btp_state.links, officials, clubs, districts, bm, match_ids_on_court).then(match => {
				...
				if (cur_match) {
					...
					// Preserve BTS state for finished matches
					if (cur_match.setup.state === 'finished') {
						match.setup.state = cur_match.setup.state;
						match.setup.now_on_court = false;
					} else if (!match.setup.now_on_court && cur_match.setup.now_on_court) {
						// BTP dragged match off court — clear all call state
						match.setup.called_timestamp = undefined;
					}
					...
					if (match.setup.now_on_court) {
						for (const k of ['called_to_court', 'called_to_court_at', 'second_call_at', 'final_call_at', 'teams_present', 'team1_present', 'team2_present']) {
							if (cur_match.setup[k] !== undefined) {
								match.setup[k] = cur_match.setup[k];
							}
						}
					}

					// Auto-call when courts-to-call is disabled
					if (auto_call && match.setup.now_on_court) {
						if (!match.setup.called_to_court) {
							match.setup.called_to_court = true;
							match.setup.called_to_court_at = Date.now();
						}
						if (!match.setup.second_call_at && match.setup.called_to_court_at) {
							const elapsed = Date.now() - match.setup.called_to_court_at;
							if (elapsed >= second_call_ms) {
								match.setup.second_call_at = match.setup.called_to_court_at + second_call_ms;
							}
						}
						if (!match.setup.final_call_at && match.setup.second_call_at) {
							const elapsed = Date.now() - match.setup.second_call_at;
							if (elapsed >= final_call_ms) {
								match.setup.final_call_at = match.setup.second_call_at + final_call_ms;
							}
						}
					}
				...
				// Auto-call new matches when courts-to-call is disabled
				if (auto_call && match.setup.now_on_court && !match.setup.called_to_court) {
					match.setup.called_to_court = true;
					match.setup.called_to_court_at = Date.now();
				}
				changes = true;
		...
		if(changes){
			...
			bupws.handle_score_change_all_courts(app, tkey);
			setTimeout(function(){
...
function integrate_courts(app, tournament_key, btp_state, location_map, callback) {
	...
	async.each(courts, (c, cb) => {
		...
		// Remove manually-added courts that have no btp_id (not in BTP)
		app.db.courts.remove({tournament_key, btp_id: {$exists: false}}, {multi: true}, (err) => {
