...
var ctournament = (function() {
	...
	function _handle_match_add_upcoming(c) {
		const match_id = c.val.match__id;
		if (!utils.find(curt.matches, m => m._id === match_id)) {
			curt.matches.push(c.val.match);
			curt.matches = curt.matches.sort((a, b) => cmatch.cmp_match_order(a, b));
		}
		const upcoming_el = document.querySelector('.upcoming_container');
		if (upcoming_el) cmatch.render_upcoming_matches(upcoming_el);
		const courts_el = document.querySelector('.courts_container');
		if (courts_el) cmatch.render_courts(courts_el, 'public');
	}
	...
	function ui_show() {
		...
		toprow.set([{
			...
			href: '/bupdev/#btsh_e=' + encodeURIComponent(curt.key) + '&display' + bup_dm_style + bup_lang,
		}, {
			label: ci18n('Umpire Panel'),
			href: '/bupdev/#btsh_e=' + encodeURIComponent(curt.key) + bup_lang,
		}, {
			...
			label: 'T-Shirt Größen',
			href: '/h/' + encodeURIComponent(curt.key) + '/tshirt-overview',
		}, {
			label: 'Rotating Display (Halle 1)',
			href: '/h/' + encodeURIComponent(curt.key) + '/rotating-display?style=ostbek1',
		}, {
			label: 'Rotating Display (Halle 2)',
			href: '/h/' + encodeURIComponent(curt.key) + '/rotating-display?style=ostbek2',
		}]);
	...
	function ui_edit() {
		...
		// tournament-flow-div##################################################################################
		{
			...
			// only-now-on-court
			const matchrules_fieldset = uiu.el(tournament_flow_div, 'fieldset');
			input.only_now_on_court = create_checkbox(curt, matchrules_fieldset, 'only_now_on_court');

			// Court Calling — rendered as a standalone .settings card to match new UI pattern
			const calling_div = uiu.el(form, 'div', 'settings');
			uiu.el(calling_div, 'h2', 'edit', ci18n('tournament:edit:calling:legend'));

			const ctc_enabled_label = uiu.el(calling_div, 'label');
			const ctc_attrs = {type: 'checkbox', name: 'courts_to_call_enabled'};
			if (curt.courts_to_call_enabled) ctc_attrs.checked = 'checked';
			const ctc_input = uiu.el(ctc_enabled_label, 'input', ctc_attrs);
			uiu.el(ctc_enabled_label, 'span', {}, ci18n('tournament:edit:calling:enabled'));
			input.courts_to_call_enabled = ctc_input;

			const sc_enabled_label = uiu.el(calling_div, 'label');
			const sc_attrs = {type: 'checkbox', name: 'second_call_enabled'};
			if (curt.second_call_enabled !== false) sc_attrs.checked = 'checked';
			const sc_input = uiu.el(sc_enabled_label, 'input', sc_attrs);
			uiu.el(sc_enabled_label, 'span', {}, ci18n('tournament:edit:calling:second_call_enabled'));
			input.second_call_enabled = sc_input;

			const sc_s_label = uiu.el(calling_div, 'label');
			uiu.el(sc_s_label, 'span', {}, ci18n('tournament:edit:calling:second_call_s'));
			const sc_s_input = uiu.el(sc_s_label, 'input', {
				type: 'number', name: 'second_call_s', min: '30', max: '3600',
				value: String(curt.second_call_s || 420),
			});
			input.second_call_s = sc_s_input;

			const fc_enabled_label = uiu.el(calling_div, 'label');
			const fc_attrs = {type: 'checkbox', name: 'final_call_enabled'};
			if (curt.final_call_enabled !== false) fc_attrs.checked = 'checked';
			const fc_input = uiu.el(fc_enabled_label, 'input', fc_attrs);
			uiu.el(fc_enabled_label, 'span', {}, ci18n('tournament:edit:calling:final_call_enabled'));
			input.final_call_enabled = fc_input;

			const fc_s_label = uiu.el(calling_div, 'label');
			uiu.el(fc_s_label, 'span', {}, ci18n('tournament:edit:calling:final_call_s'));
			const fc_s_input = uiu.el(fc_s_label, 'input', {
				type: 'number', name: 'final_call_s', min: '30', max: '3600',
				value: String(curt.final_call_s || 300),
			});
			input.final_call_s = fc_s_input;

			// disable sub-options when courts_to_call is off
			const calling_sub_inputs = [sc_input, sc_s_input, sc_enabled_label, fc_input, fc_s_input, fc_enabled_label];
			function update_calling_sub() {
				const enabled = ctc_input.checked;
				calling_sub_inputs.forEach(function(el) {
					if (enabled) { el.removeAttribute('disabled'); }
					else { el.setAttribute('disabled', 'disabled'); }
				});
			}
			ctc_input.addEventListener('change', update_calling_sub);
			update_calling_sub();
			...
			const tshirt_label = uiu.el(calling_div, 'label');
			const tshirt_attrs = {type: 'checkbox', name: 'tshirt_enabled'};
			if (curt.tshirt_enabled) tshirt_attrs.checked = 'checked';
			const tshirt_input = uiu.el(tshirt_label, 'input', tshirt_attrs);
			uiu.el(tshirt_label, 'span', {}, 'T-Shirt Größenabfrage im Halbfinale');
			input.tshirt_enabled = tshirt_input;

			const tshirt_sizes_label = uiu.el(calling_div, 'label');
			uiu.el(tshirt_sizes_label, 'span', {}, 'T-Shirt Größen (kommagetrennt)');
			const tshirt_sizes_input = uiu.el(tshirt_sizes_label, 'input', {
				type: 'text', name: 'tshirt_sizes',
				value: curt.tshirt_sizes || 'XS,S,M,L,XL',
				placeholder: 'XS,S,M,L,XL',
			});
			input.tshirt_sizes = tshirt_sizes_input;

			function update_tshirt_sub() {
				const enabled = tshirt_input.checked;
				if (enabled) { tshirt_sizes_input.removeAttribute('disabled'); }
				else { tshirt_sizes_input.setAttribute('disabled', 'disabled'); }
			}
			tshirt_input.addEventListener('change', update_tshirt_sub);
			update_tshirt_sub();
...
// officials_host ######################################################################################################
	...
	function send_props(input, callback) {
		const props = {
			...
			only_now_on_court: input.only_now_on_court.checked,
			courts_to_call_enabled: input.courts_to_call_enabled.checked,
			second_call_enabled: input.second_call_enabled.checked,
			second_call_s: parseInt(input.second_call_s.value, 10) || 420,
			final_call_enabled: input.final_call_enabled.checked,
			final_call_s: parseInt(input.final_call_s.value, 10) || 300,
			tshirt_enabled: input.tshirt_enabled.checked,
			tshirt_sizes: input.tshirt_sizes.value.trim() || 'XS,S,M,L,XL',
			court_free_sound: input.court_free_sound.value,
		}
...
function update_officials() {
	...
	_route_single(/t\/([a-z0-9]+)\/upcoming/, ui_upcoming, change.default_handler(_update_all_ui_elements_upcoming, {
		...
		match_add: _handle_match_add_upcoming,
		update_player_status: update_player_status,
	}));
